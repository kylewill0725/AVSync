...
namespace AVSync
{
    public class BassEngine : IWaveformPlayer, ISpectrumPlayer
    {
        ...
        public static BassEngine Instance => instance ?? (instance = new BassEngine());
        ...
        public void Stop()
        {
            ...
            {
                Bass.ChannelStop(ActiveStreamHandle);
                Bass.ChannelSetPosition(ActiveStreamHandle, ChannelPosition);
            }
            IsPlaying = false;
            CanStop = false;
            CanPlay = true;
        ...
        public void Pause()
        {
            ...
            {
                Bass.ChannelPause(ActiveStreamHandle);
                IsPlaying = false;
                CanPlay = true;
                CanPause = false;
            }
        ...
        public bool OpenFile(string path)
        {
            ...
            {
                // Create Stream
                FileStreamHandle = ActiveStreamHandle = Bass.CreateStream(path, 0, 0, BassFlags.Prescan | BassFlags.Float);
                ChannelLength = Bass.ChannelBytes2Seconds(FileStreamHandle, Bass.ChannelGetLength(FileStreamHandle));
                FileTag = TagLib.File.Create(path);
                GenerateWaveformData(path);
                if (ActiveStreamHandle != 0)
                {
                    // Obtain the sample rate of the stream
                    Bass.ChannelGetInfo(ActiveStreamHandle, out var info);
                    sampleFrequency = info.Frequency;

                    // Set the stream to call Stop() when it ends.
                    int syncHandle = Bass.ChannelSetSync(ActiveStreamHandle,
                         SyncFlags.End,
                         0,
                         endTrackSyncProc);
        ...
        private void positionTimer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ChannelPosition = Bass.ChannelBytes2Seconds(ActiveStreamHandle, Bass.ChannelGetPosition(ActiveStreamHandle, 0));
        ...
        private void waveformGenerateWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            int stream = Bass.CreateStream(waveformParams.Path, 0, 0, BassFlags.Decode | BassFlags.Float | BassFlags.Prescan);
            int frameLength = (int)Bass.ChannelBytes2Seconds(stream, 0.02);
